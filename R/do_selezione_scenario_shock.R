...
do_selezione_scenario_shock <- function(.curve_1y_interpol, .shock_effettivi, .prepayment, .scenario_no_prepayment, .mesi_tenor_prepayment, .n_core) {
  ...
  if(.prepayment == "SI"){
    ...
    curve_1y_interpol_tenor_1 <- .curve_1y_interpol %>%
      filter (ID_MESE_MAT == .mesi_tenor_prepayment , ID_SCEN != 0) %>%
      select(ID_YEAR, COD_VALUTA , ID_SCEN, ID_SCEN_CLASS, ID_MESE_MAT, VAL_TASSO_1 = VAL_TASSO)
    ...
    scenari_prep <- scenari_prep %>%
      ...
      select(ID_YEAR, COD_VALUTA, ID_SCEN, ID_SCEN_CLASS, ID_MESE_MAT, SHOCK_SIMULATO)
  ...
  return(list(scenari_prep = scenari_prep2, scenari_noprep = scenari_noprep))
